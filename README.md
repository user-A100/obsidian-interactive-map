...
The runtime plugin stays lightweight and does **not** bundle map assets. Put your SVG and its same-name JSON sidecar in a normal Vault folder, such as `归档/attachment/`.

Create a code block in any note:

~~~
```interactive-map
[[归档/attachment/china_provinces_map.svg]]
```
~~~
...
The source repository includes two optional Node.js scripts under `scripts/` that generate SVG maps and JSON sidecars from public DataV GeoJSON data. These scripts are development tools and are not required at plugin startup.
...
插件运行目录保持轻量，不再内置地图附件。请将 SVG 和同名 JSON 侧车文件放入 Vault 的普通附件目录，例如 `归档/attachment/`。
